...
Intro_Window *IntroWindowCreate()
{
    ...
    win->windowIntro = SDL_CreateWindow("INSTRUCTIONS", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, INTRO_WINDOW_WIDTH, INTRO_WINDOW_HEIGHT, SDL_WINDOW_OPENGL | SDL_WINDOW_RESIZABLE | SDL_WINDOW_SHOWN);
...
void IntroWindowDraw(Intro_Window *src)
{
    ASSERT(src != NULL);
    //Drawing Background
    SDL_Surface *surface = IMG_Load("./resources/images/pic/instructionWindow.png");
    if (!surface)
    {
